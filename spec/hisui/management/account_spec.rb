describe Hisui::Management::Account do
  let!(:user) { Hisui::User.new(access_token) }
  let!(:account) { Hisui::Management::Account.new({'id' => 54516992, 'name' => 'Demo Account (Beta)'}, user) }
  ...
  context 'Class methods' do
    ...
    context '#build_from_summary' do
      ...
      let!(:account_from_summary) { Hisui::Management::Account.build_from_summary(summary_attributes, user) }
    ...
    context '#from_child' do
      let!(:account_from_child) { Hisui::Management::Account.from_child(web_property) }
